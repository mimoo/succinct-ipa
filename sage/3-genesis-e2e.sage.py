...
# This file was *autogenerated* from the file sage/3-genesis-e2e.sage
from sage.all_cmdline import *   # import sage library

_sage_const_28948022309329048855892746252171976963363056481941560715954676764349967630337 = Integer(28948022309329048855892746252171976963363056481941560715954676764349967630337); _sage_const_28948022309329048855892746252171976963363056481941647379679742748393362948097 = Integer(28948022309329048855892746252171976963363056481941647379679742748393362948097); _sage_const_0 = Integer(0); _sage_const_5 = Integer(5); _sage_const_15 = Integer(15); _sage_const_32 = Integer(32); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_8 = Integer(8); _sage_const_16 = Integer(16); _sage_const_224 = Integer(224); _sage_const_3 = Integer(3); _sage_const_10 = Integer(10); _sage_const_255 = Integer(255); _sage_const_4 = Integer(4); _sage_const_7 = Integer(7); _sage_const_100000 = Integer(100000); _sage_const_11 = Integer(11); _sage_const_275 = Integer(275); _sage_const_9 = Integer(9); _sage_const_36 = Integer(36); _sage_const_430 = Integer(430); _sage_const_379 = Integer(379); _sage_const_228 = Integer(228); _sage_const_1000 = Integer(1000); _sage_const_42 = Integer(42); _sage_const_12 = Integer(12)# 3-genesis-e2e.sage — END-TO-END Genesis on the Pallas curve.
#
...
NROUNDS = _sage_const_8 
RC = [Fp(int(hashlib.sha256(f"genesis-rc|{t}".encode()).hexdigest(), _sage_const_16 ))
...
def derive_native(seed_fe, i):
    ...
    gs = [x**_sage_const_3  + _sage_const_5  for x in cands]
    vs = [g**E1BITS_EXP for g in gs]         # g^((m+1)/2)
    ws = [g**E2BITS_EXP for g in gs]         # g^m
    ls = [w**(_sage_const_2 **(S2AD - _sage_const_1 )) for w in ws]    # Legendre symbol = w^(2^31)
    qs = [(_sage_const_1  + l) * INV2 for l in ls]
    sels, acc = [], Fp(_sage_const_1 )
    for c in range(CAND):
        sels.append(qs[c] * acc); acc *= (_sage_const_1  - qs[c])
    if sum(sels) != _sage_const_1 :
    ...
    assert v * v == x**_sage_const_3  + _sage_const_5 
    return (x, v)
...
def setup(seed_str, n):
    ...
    while True:
        ...
        if all(pt is not None for pt in pts) and U is not None:
            ...
            return seed_fe, G, E(U[_sage_const_0 ], U[_sage_const_1 ])
        ctr += _sage_const_1 
...
def build_circuit(k, seed_fe, xs, prover):
    ...
    for t in range(NROUNDS):
        rc = RC[t]
        B.simd([("u2", [h], (lambda d, _h=h, _rc=rc: (d[_h] + _rc)**_sage_const_2 ), _sage_const_2 ),
                ident(h)])
        B.simd([("u4", ["u2"], (lambda d: d["u2"]**_sage_const_2 ), _sage_const_2 ), ident(h)])
        B.simd([("h", ["u4", h],
                 (lambda d, _h=h, _rc=rc: d["u4"] * (d[_h] + _rc)), _sage_const_2 )])
        h = "h"

    # ---- stage 2: candidates and g_c = x_c^3 + 5 ---------------------------
    B.simd([(f"sq{c}", ["h"], (lambda d, _c=c: (d["h"] + _c)**_sage_const_2 ), _sage_const_2 )
            for c in range(CAND)] + [ident("h")])
    B.simd([(f"g{c}", [f"sq{c}", "h"],
             (lambda d, _c=c: d[f"sq{_c}"] * (d["h"] + _c) + _sage_const_5 ), _sage_const_2 )
            for c in range(CAND)] + [ident("h")])

    # ---- stage 3: exponent chains v_c = g^((m+1)/2), w_c = g^m -------------
    B.simd([(f"v{c}", [], (lambda d: Fp(_sage_const_1 )), _sage_const_0 ) for c in range(CAND)] +
           [(f"w{c}", [], (lambda d: Fp(_sage_const_1 )), _sage_const_0 ) for c in range(CAND)] +
            ...
            for c in range(CAND):
                if E1BITS[t]:
                    outs.append((f"v{c}", [f"v{c}", f"g{c}"],
                                 (lambda d, _c=c: d[f"v{_c}"] * d[f"g{_c}"]), _sage_const_2 ))
                else:
                    outs.append(ident(f"v{c}"))
                if E2BITS[t]:
                    outs.append((f"w{c}", [f"w{c}", f"g{c}"],
                                 (lambda d, _c=c: d[f"w{_c}"] * d[f"g{_c}"]), _sage_const_2 ))
                else:
    ...
    B.simd([(f"l{c}", [f"w{c}"], (lambda d, _c=c: d[f"w{_c}"]**_sage_const_2 ), _sage_const_2 )
            for c in range(CAND)] + carry2)
    for _ in range(S2AD - _sage_const_2 ):
        B.simd([(f"l{c}", [f"l{c}"], (lambda d, _c=c: d[f"l{_c}"]**_sage_const_2 ), _sage_const_2 )
                for c in range(CAND)] + carry2)

    # ---- stage 5: select first QR candidate (iterative, deg-2 layers;
    #      "not-yet-selected" running product np, selector s_c = q_c*np) -----
    def qf(l): return (_sage_const_1  + l) * INV2
    carry_l = [ident(f"l{c}") for c in range(CAND)]
    B.simd([("np", [], (lambda d: Fp(_sage_const_1 )), _sage_const_0 )] + carry_l + carry2)
    for c in range(CAND):
        prev_s = [ident(f"s{j}") for j in range(c)]
        keep_l = [ident(f"l{j}") for j in range(c + _sage_const_1 , CAND)]
        B.simd([(f"s{c}", ["np", f"l{c}"],
                 (lambda d, _c=c: qf(d[f"l{_c}"]) * d["np"]), _sage_const_2 ),
                ("np", ["np", f"l{c}"],
                 (lambda d, _c=c: d["np"] * (_sage_const_1  - qf(d[f"l{_c}"]))), _sage_const_2 )]
               + prev_s + keep_l + carry2)
    all_s = [f"s{c}" for c in range(CAND)]
    B.simd([("xs", all_s + ["h"],
             (lambda d: sum(d[f"s{c}"] * (d["h"] + c)
                            for c in range(CAND))), _sage_const_2 ),
    ...
    B.simd([("X", ["xs"], (lambda d: d["xs"]), _sage_const_1 ),
            ("Y", ["vv"], (lambda d: d["vv"]), _sage_const_1 ),
            ("Z", [], (lambda d: Fp(_sage_const_1 )), _sage_const_0 )])
    for j in range(k):
