...
def derive_native(seed_fe, i):
    ...
    l = w1**(_sage_const_2 **(S2AD - _sage_const_1 ))                  # Legendre(gx1) in {1,-1}
    s = (_sage_const_1  + l) * INV2
    x = s * x1 + (_sage_const_1  - s) * x2
    v = s * v1 + (_sage_const_1  - s) * v2
    w = s * w1 + (_sage_const_1  - s) * w2
    for K in range(S2AD - _sage_const_1 , _sage_const_0 , -_sage_const_1 ):         # constant-time Tonelli-Shanks
        t = w**(_sage_const_2 **(K - _sage_const_1 ))
        b = (_sage_const_1  - t) * INV2
        w = w * (_sage_const_1  + b * (ZPOW[S2AD - K] - _sage_const_1 ))
        v = v * (_sage_const_1  + b * (ZPOW[S2AD - _sage_const_1  - K] - _sage_const_1 ))
    y = v
...
def build_circuit(k, seed_fe, xs, prover):
    ...
    for r in range(PRF + PRP):
        ...
        B.simd([(f"p{i}", ["p0", "p1", "p2"], mkf(i), _sage_const_5 ) for i in range(_sage_const_3 )])

    # ---- stage 2: SWU on the iso curve (u = p0) -----------------------------
    B.simd([("u2", ["p0"], (lambda d: d["p0"]**_sage_const_2 ), _sage_const_2 )])
    B.simd([("u4", ["u2"], (lambda d: d["u2"]**_sage_const_2 ), _sage_const_2 ), ident("u2")])
    B.simd([("den", ["u4", "u2"],
             (lambda d: SWU_Z**_sage_const_2  * d["u4"] + SWU_Z * d["u2"]), _sage_const_1 ),
            ident("u2")])
    # in-circuit inversion tv1 = den^(p-2), double-and-multiply
    B.simd([("iv", [], (lambda d: Fp(_sage_const_1 )), _sage_const_0 ), ident("den"), ident("u2")])
    for t in range(_sage_const_255 ):
        B.simd([("iv", ["iv"], (lambda d: d["iv"]**_sage_const_2 ), _sage_const_2 ),
                ident("den"), ident("u2")])
        if PM2BITS[t]:
            B.simd([("iv", ["iv", "den"], (lambda d: d["iv"] * d["den"]), _sage_const_2 ),
                    ident("den"), ident("u2")])
    B.simd([("x1", ["iv"], (lambda d: MBA * (_sage_const_1  + d["iv"])), _sage_const_1 ), ident("u2")])
    ...
    B.simd([(f"v{c}", [], (lambda d: Fp(_sage_const_1 )), _sage_const_0 ) for c in (_sage_const_1 , _sage_const_2 )] +
           [(f"w{c}", [], (lambda d: Fp(_sage_const_1 )), _sage_const_0 ) for c in (_sage_const_1 , _sage_const_2 )] + carry)
    for t in range(_sage_const_224 ):
        B.simd([(f"v{c}", [f"v{c}"], (lambda d, _c=c: d[f"v{_c}"]**_sage_const_2 ), _sage_const_2 )
                for c in (_sage_const_1 , _sage_const_2 )] +
               [(f"w{c}", [f"w{c}"], (lambda d, _c=c: d[f"w{_c}"]**_sage_const_2 ), _sage_const_2 )
                for c in (_sage_const_1 , _sage_const_2 )] + carry)
        if E1BITS[t] or E2BITS[t]:
            outs = []
            for c in (_sage_const_1 , _sage_const_2 ):
                if E1BITS[t]:
                    outs.append((f"v{c}", [f"v{c}", f"gx{c}"],
                                 (lambda d, _c=c: d[f"v{_c}"] * d[f"gx{_c}"]), _sage_const_2 ))
                else:
                    outs.append(ident(f"v{c}"))
                if E2BITS[t]:
                    outs.append((f"w{c}", [f"w{c}", f"gx{c}"],
                                 (lambda d, _c=c: d[f"w{_c}"] * d[f"gx{_c}"]), _sage_const_2 ))
                else:
                    outs.append(ident(f"w{c}"))
            B.simd(outs + carry)

    # ---- stage 4: Legendre l = w1^(2^31) in {1,-1} --------------------------
    carry2 = ([ident(f"v{c}") for c in (_sage_const_1 , _sage_const_2 )] +
              [ident(f"w{c}") for c in (_sage_const_1 , _sage_const_2 )] + [ident("x1"), ident("x2")])
    B.simd([("l", ["w1"], (lambda d: d["w1"]**_sage_const_2 ), _sage_const_2 )] + carry2)
    for _ in range(S2AD - _sage_const_2 ):
        B.simd([("l", ["l"], (lambda d: d["l"]**_sage_const_2 ), _sage_const_2 )] + carry2)

    # ---- stage 5: select the square branch (SWU guarantees one) -------------
    def sel(a, b):
        return (lambda d, _a=a, _b=b:
                (_sage_const_1  + d["l"]) * INV2 * d[_a]
                + (_sage_const_1  - (_sage_const_1  + d["l"]) * INV2) * d[_b])
    B.simd([("xx", ["l", "x1", "x2"], sel("x1", "x2"), _sage_const_2 ),
            ("vv", ["l", "v1", "v2"], sel("v1", "v2"), _sage_const_2 ),
            ("ww", ["l", "w1", "w2"], sel("w1", "w2"), _sage_const_2 )])

    # ---- stage 6: constant-time Tonelli-Shanks sqrt (y = vv) ----------------
    for K in range(S2AD - _sage_const_1 , _sage_const_0 , -_sage_const_1 ):
        if K > _sage_const_1 :
            B.simd([("t", ["ww"], (lambda d: d["ww"]**_sage_const_2 ), _sage_const_2 ),
                    ident("ww"), ident("vv"), ident("xx")])
            for _ in range(K - _sage_const_2 ):
                B.simd([("t", ["t"], (lambda d: d["t"]**_sage_const_2 ), _sage_const_2 ),
                        ident("ww"), ident("vv"), ident("xx")])
            tdep = "t"
        else:
            tdep = "ww"
        z1, z2 = ZPOW[S2AD - K], ZPOW[S2AD - _sage_const_1  - K]
        B.simd([("ww", ["ww", tdep],
                 (lambda d, _t=tdep, _z=z1:
                  d["ww"] * (_sage_const_1  + (_sage_const_1  - d[_t]) * INV2 * (_z - _sage_const_1 ))), _sage_const_2 ),
                ("vv", ["vv", tdep],
                 (lambda d, _t=tdep, _z=z2:
                  d["vv"] * (_sage_const_1  + (_sage_const_1  - d[_t]) * INV2 * (_z - _sage_const_1 ))), _sage_const_2 ),
                ident("xx")])
    ...
    B.simd([("X", ["nx", "dyv"], (lambda d: d["nx"] * d["dyv"]), _sage_const_2 ),
            ("Y", ["vv", "ny1", "dxv"],
             (lambda d: d["vv"] * d["ny1"] * d["dxv"]), _sage_const_3 ),
