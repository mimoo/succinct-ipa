...
E = EllipticCurve(GF(p), [_sage_const_0 , _sage_const_5 ])            # Pallas: y^2 = x^3 + 5, #E = q
Fq = GF(q)                                  # scalar field
INF = E(_sage_const_0 )
assert E.order() == q
assert (q - _sage_const_1 ) % (_sage_const_2 **_sage_const_20 ) == _sage_const_0                # enough 2-adicity for our sizes
...
class Cost:
    def __init__(self):
        self.smul = _sage_const_0         # group scalar multiplications
        self.hash = _sage_const_0         # sha256 invocations
...
def s_vector(xs, k):
    ...
    for i in range(n):
        v = Fq(_sage_const_1 )
        for j in range(k):
            v *= xs[j] if (i >> j) & _sage_const_1  else xs[j]**-_sage_const_1 
        s.append(v)
    return s

def mleG(G, y):
    # monomial multilinear:  sum_t (prod_{j in t} y_j) * G_t     (Lean: Experiments.mleG)
    k = len(y); n = _sage_const_1  << k
    acc = INF
    for t in range(n):
        c = Fq(_sage_const_1 )
        for j in range(k):
            if (t >> j) & _sage_const_1 :
                c *= y[j]
        acc += int(c) * G[t]
    return acc
...
def eqW(r, b_bits, k):
    v = Fq(_sage_const_1 )
    for j in range(k):
        v *= r[j] if (b_bits >> j) & _sage_const_1  else (_sage_const_1  - r[j])
    return v
...
# ----------------------------------------------------------------------------
def leaf_hash(P):
    tag = b"INF" if P == INF else int(P[_sage_const_0 ]).to_bytes(_sage_const_32 , "big") + int(P[_sage_const_1 ]).to_bytes(_sage_const_32 , "big")
...
def merkle_tree(points):
    ...
    while len(level) > _sage_const_1 :
        nxt = [hashlib.sha256(level[i] + level[i + _sage_const_1 ]).digest()
...
def merkle_path(levels, idx):
    ...
    for lvl in levels[:-_sage_const_1 ]:
        path.append(lvl[idx ^ _sage_const_1 ])          # ^^ is XOR in Sage (^ is exponentiation)
        idx >>= _sage_const_1 
    return path
...
def decide_prove(G, y, k):
    ...
    for i in range(k):                        # k folds  ->  length dom/2^k = 2
        ...
        w = w * w
    M = layers[-_sage_const_1 ][_sage_const_0 ]                         # final constant  = mleG(G, y)
    return {"layers": layers, "trees": roots,
            "roots": [t[-_sage_const_1 ][_sage_const_0 ] for t in roots], "M": M, "omega": omega}

NUM_QUERIES = _sage_const_20                               # demo soundness ~ (1/2)^20; real uses ~lambda

def decide_verify(proof, y, k, cost, tampered_layers=None):
    dom = _sage_const_2  * (_sage_const_1  << k)
    omega = proof["omega"]
    ...
    for _ in range(NUM_QUERIES):
        ...
        w = omega
        for i in range(k):
            m = len(layers[i]); half = m // _sage_const_2 
            t = pos % half
    ...
    if not merkle_check(proof["roots"][k], _sage_const_0 , layers[k][_sage_const_0 ], merkle_path(trees[k], _sage_const_0 ), cost):
        return False, None
    return True, proof["M"]
...
def ipa_prove(seed, G, U, a, z):
    ...
    while len(aa) > _sage_const_1 :
        ...
        x = tr.challenge("x"); xs.append(x); xi = x**-_sage_const_1 
...
def b0_closed(xs, z, k):
    ...
    for j in range(k):
        val *= xs[j]**-_sage_const_1  + xs[j] * z**(_sage_const_2 **j)
    return val
...
def prism_verify(G, U, proof, decide):
    ...
    cost = Cost()
    k = proof["k"]; xb = proof["xb"]; a = proof["a"]
    y = [xb[j]**_sage_const_2  for j in range(k)]                      # eval point (genFinal_eq_mle)
    ok_fri, M = decide_verify(decide, y, k, cost)         # <-- polylog, no n-MSM
    if not ok_fri:
        return False, None, cost, "decide rejected"
    prod_inv = Fq(_sage_const_1 )
    for j in range(k):
        prod_inv *= xb[j]**-_sage_const_1 
...
def run(seed, k, do_checks=True):
    ...
    decide = decide_prove(G, [proof["xb"][j]**_sage_const_2  for j in range(k)], k)
    ...
    if do_checks:
        ...
        y = [proof["xb"][j]**_sage_const_2  for j in range(k)]
        prod_inv = prod([proof["xb"][j]**-_sage_const_1  for j in range(k)], Fq(_sage_const_1 ))
        ...
        vec = list(G)
        for j in range(k):
            vec = [int(_sage_const_1  - r[j]) * vec[_sage_const_2 *i] + int(r[j]) * vec[_sage_const_2 *i+_sage_const_1 ] for i in range(len(vec)//_sage_const_2 )]
        eq_msm = sum((int(eqW(r, b, k)) * G[b] for b in range(n)), INF)
        assert vec[_sage_const_0 ] == eq_msm                                  # foldAll_eq_mleEval
...
def main():
    ...
    k = _sage_const_6 
    print(f"[run]     end-to-end at n = 2^{k} = {_sage_const_1 <<k}")
    ...
    bad_layers[_sage_const_0 ] = [P + G[_sage_const_0 ] for P in bad_layers[_sage_const_0 ]]   # values no longer match the root
    y = [proof["xb"][j]**_sage_const_2  for j in range(k)]
    ...
    bad = dict(decide); bad["M"] = decide["M"] + G[_sage_const_0 ]
    ...
    for kk in (_sage_const_6 , _sage_const_10 , _sage_const_12 ):
