# 7-exodus.sage — EXODUS: Genesis without the lambda-grind (no FRI, no hashes).
...
def derive_native(seed_fe, i):
    ...
    if den == 0:
        return None
    tv1 = den**(p - 2)                       # ADVICE: the inverse
    x1 = MBA * (1 + tv1)
    ...
    if gx1 == 0 or gx2 == 0:
        return None
    l = gx1**((p - 1) // 2)                  # Legendre(gx1) in {1,-1}
    s = (1 + l) * INV2                       # ADVICE: the branch bit
    x = s * x1 + (1 - s) * x2
    gxsel = s * gx1 + (1 - s) * gx2
...
def setup(seed_str, n):
    ...
    while True:
        ...
        if all(pt is not None for pt in pts) and U is not None:
            G = [from_proj(pt[0], pt[1], pt[2]) for pt in pts]
            adv = {"tv": [pt[3] for pt in pts],
                   "s":  [pt[4] for pt in pts],
                   "y":  [pt[5] for pt in pts]}
            adv["C"] = {c: msm(adv[c], G) for c in ("tv", "s", "y")}
            return seed_fe, G, from_proj(U[0], U[1], U[2]), adv
        ctr += 1
...
class Builder:
    ...
    def assert_zero(self, col):
        self.asserts.setdefault(len(self.steps) - 1, []).append(col)
    def simd(self, outs):
...
def inject_asserts(claims, asserts, fi, step, tr):
    ...
    for col in asserts[fi]:
        ...
        claims.setdefault(col, []).append((pt, Fp(0)))
...
def gkr_prove(steps, out_claims, tr, debug=False, asserts=None):
    ...
    for step in reversed(steps):
        widx += 1
        inject_asserts(claims, asserts, nsteps - widx - 1, step, tr)
        if step["kind"] == "split":
            new = {}
...
def build_circuit(k, seed_fe, xs, prover, advice=None):
    ...
    for r in range(PRF + PRP):
        ...
        def mkf(i, _rc=PRC[r], _full=full):
            ...
            return (lambda d, _i=i, _c=_rc:
                    ...
                    + PMDS[_i][1] * (d["p1"] + _c[1])
                    + PMDS[_i][2] * (d["p2"] + _c[2]))
        B.simd([(f"p{i}", ["p0", "p1", "p2"], mkf(i), 5)
                for i in range(3)] + adv_carry)

    # ---- stage 2': SWU with ADVICE (tv = 1/den) — chains deleted ------------
    B.simd([("u2", ["p0"], (lambda d: d["p0"]**2), 2)] + adv_carry)
    B.simd([("u4", ["u2"], (lambda d: d["u2"]**2), 2),
            ident("u2")] + adv_carry)
    B.simd([("den", ["u4", "u2"],
             (lambda d: SWU_Z**2 * d["u4"] + SWU_Z * d["u2"]), 1),
            ident("u2")] + adv_carry)
    B.simd([("chkinv", ["tv", "den"],
             (lambda d: d["tv"] * d["den"] - 1), 2),
            ident("u2")] + adv_carry)
    B.assert_zero("chkinv")                       # tv·den = 1
    B.simd([("x1", ["tv"], (lambda d: MBA * (1 + d["tv"])), 1),
    ...
    keep = [ident("x1"), ident("x2"), ident("gx1"), ident("gx2"),
            ident("s"), ident("y")]
    B.simd([("chks", ["s"], (lambda d: d["s"] * (d["s"] - 1)), 2)] + keep)
    B.assert_zero("chks")                         # s ∈ {0,1}
    B.simd([("gxsel", ["s", "gx1", "gx2"],
             (lambda d: d["s"] * d["gx1"] + (1 - d["s"]) * d["gx2"]), 2),
            ident("x1"), ident("x2"), ident("s"), ident("y")])
    B.simd([("chky", ["y", "gxsel"],
             (lambda d: d["y"] * d["y"] - d["gxsel"]), 2),
            ident("x1"), ident("x2"), ident("s"), ident("y")])
    B.assert_zero("chky")                         # y² = selected g(x)
    B.simd([("xx", ["s", "x1", "x2"],
             (lambda d: d["s"] * d["x1"] + (1 - d["s"]) * d["x2"]), 2),
            ident("y")])
