...
def derive_native(seed_fe, i):
    ...
    cands = [h + c for c in range(_sage_const_4 )]
    ...
    for c in range(_sage_const_4 ):
        sels.append(qs[c] * acc); acc *= (_sage_const_1  - qs[c])
    if sum(sels) != _sage_const_1 :
        return None                          # no QR candidate: reject seed
    x = sum(sels[c] * cands[c] for c in range(_sage_const_4 ))
    v = sum(sels[c] * vs[c] for c in range(_sage_const_4 ))
    w = sum(sels[c] * ws[c] for c in range(_sage_const_4 ))
...
def ident(name):
    return (name, [name], (lambda d, _n=name: d[_n]), _sage_const_1 )
...
def build_circuit(k, seed_fe, xs, prover):
    ...
    B.simd([(f"sq{c}", ["h"], (lambda d, _c=c: (d["h"] + _c)**_sage_const_2 ), _sage_const_2 )
            for c in range(_sage_const_4 )] + [ident("h")])
    ...
    B.simd([(f"v{c}", [], (lambda d: Fp(_sage_const_1 )), _sage_const_0 ) for c in range(_sage_const_4 )] +
           [(f"w{c}", [], (lambda d: Fp(_sage_const_1 )), _sage_const_0 ) for c in range(_sage_const_4 )] +
           [ident(f"g{c}") for c in range(_sage_const_4 )] + [ident("h")])
    carry = [ident(f"g{c}") for c in range(_sage_const_4 )] + [ident("h")]
    for t in range(_sage_const_224 ):
        B.simd([(f"v{c}", [f"v{c}"], (lambda d, _c=c: d[f"v{_c}"]**_sage_const_2 ), _sage_const_2 )
               ...
               [(f"w{c}", [f"w{c}"], (lambda d, _c=c: d[f"w{_c}"]**_sage_const_2 ), _sage_const_2 )
                for c in range(_sage_const_4 )] + carry)
    ...
    carry2 = ([ident(f"v{c}") for c in range(_sage_const_4 )] +
              [ident(f"w{c}") for c in range(_sage_const_4 )] + [ident("h")])
    B.simd([(f"l{c}", [f"w{c}"], (lambda d, _c=c: d[f"w{_c}"]**_sage_const_2 ), _sage_const_2 )
            for c in range(_sage_const_4 )] + carry2)
    ...
    sel_fs = [
        ("s0", ["l0"], (lambda d: qf(d["l0"])), _sage_const_1 ),
        ("s1", ["l0", "l1"],
         (lambda d: qf(d["l1"]) * (_sage_const_1  - qf(d["l0"]))), _sage_const_2 ),
        ("s2", ["l0", "l1", "l2"],
         (lambda d: qf(d["l2"]) * (_sage_const_1  - qf(d["l1"])) * (_sage_const_1  - qf(d["l0"]))), _sage_const_3 ),
        ("s3", ["l0", "l1", "l2", "l3"],
         (lambda d: qf(d["l3"]) * (_sage_const_1  - qf(d["l2"])) * (_sage_const_1  - qf(d["l1"]))
                    * (_sage_const_1  - qf(d["l0"]))), _sage_const_4 ),
    ]
    B.simd(sel_fs + carry2)
    B.simd([("xs", [f"s{c}" for c in range(_sage_const_4 )] + ["h"],
             (lambda d: sum(d[f"s{c}"] * (d["h"] + c) for c in range(_sage_const_4 ))), _sage_const_2 ),
            ("vv", [f"s{c}" for c in range(_sage_const_4 )] + [f"v{c}" for c in range(_sage_const_4 )],
             (lambda d: sum(d[f"s{c}"] * d[f"v{c}"] for c in range(_sage_const_4 ))), _sage_const_2 ),
            ("ww", [f"s{c}" for c in range(_sage_const_4 )] + [f"w{c}" for c in range(_sage_const_4 )],
             (lambda d: sum(d[f"s{c}"] * d[f"w{c}"] for c in range(_sage_const_4 ))), _sage_const_2 )])
